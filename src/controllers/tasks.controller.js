...
export const createTasksPublic = async (req, res) => {
  try {
    ...
    // Crear la nueva tarea usando el cuerpo de la solicitud
    // El campo nroexpediente no se incluye aquí para que Mongoose lo
    // establezca en `null` por defecto, como se define en el esquema.
...
export const createTasks = async (req, res) => {
  try {
    const { nroexpediente, ...otherData } = req.body;
    const { user } = req;

    // Verificar si ya existe una tarea con el mismo DNI
    const existingTask = await Task.findOne({ dni: otherData.dni });
    if (existingTask) {
    ...
    const files = req.files ? req.files.map(file => ({
        ...file, // Usa el operador de propagación para mayor concisión
        id: file.id
    })) : [];
    ...
    const newTask = new Task({
      ...otherData,
      file: files,
      estado: otherData.estado || 'ingresado',
      user: user.id,
      nroexpediente: finalNroExpediente,
    });

    await newTask.save();
    await logActivity(user.id, 'creó tarea', 'tarea', newTask._id);
